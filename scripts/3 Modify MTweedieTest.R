...
#Add in function (also in MTweedieTest)
simTweedieTest <-  
  function(N){ 
    t.test( 
...
MTweedieTests <-  
  function(N,M,sig){
    ...
    res <-  foreach(
      ...
      #set.seed(i)  #Set a unique seed 
      simTweedieTest(N)<sig  #Simulate
    }
    stopCluster(cl)
    
    share_reject <- (sum(res)/M) #Calculate
...
#tic(paste0("Parallel loop, ", Cores, " cores")) #Only to check that it works
...
#toc(log = TRUE) #Only to check that it works
